...
func (this *Solution) Pick() []int {
	...
	for i, rect := range this.rects {
		...
		curSize += cur
		if rand.Intn(curSize) < cur {
			index = i
		}
	}
	x := rand.Intn(this.rects[index][2]-this.rects[index][0]+1) + this.rects[index][0]
	y := rand.Intn(this.rects[index][3]-this.rects[index][1]+1) + this.rects[index][1]
	return []int{x, y}
}
