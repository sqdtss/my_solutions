...
//给你一个长度为 n + 1 的数组 obstacles ，其中 obstacles[i] （取值范围从 0 到 3）表示在点 i 处的 obstacles[i] 跑道上有一个障碍。如果 obstacles[i] == 0 ，
//那么点 i 处没有障碍。任何一个点的三条跑道中 最多有一个 障碍。
...
func minSideJumps(obstacles []int) int {
	...
	for i := 1; i < len(obstacles); i++ {
		...
		if obstacles[i] > 0 {
			dp[i][obstacles[i]-1] = len(obstacles)
		}
		if obstacles[i-1] == 1 {
			dp[i][0] = min(dp[i][1], dp[i][2]) + 1
		} else if obstacles[i-1] == 2 {
			dp[i][1] = min(dp[i][0], dp[i][2]) + 1
		} else if obstacles[i-1] == 3 {
			dp[i][2] = min(dp[i][0], dp[i][1]) + 1
		}
	}
	return min(dp[len(obstacles)-1][0], dp[len(obstacles)-1][1], dp[len(obstacles)-1][2])
}
